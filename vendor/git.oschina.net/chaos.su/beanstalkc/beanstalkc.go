...
func (i *Client) yamlCmd(command string, dest interface{}) (err error) {
	...
	dataLen := bodyLen + 2
	rawYaml := make([]byte, dataLen)

	var n int
	var startN int64
	for startN < dataLen {
		n, err = i.ReadWriter.Read(rawYaml[startN:])
		if err != nil {
			return
		}
		startN += int64(n)
	...
	if startN != dataLen {
		err = exception(fmt.Sprintf("read only %d bytes of %d", startN, dataLen))
	}

	err = yaml.Unmarshal(rawYaml[:dataLen-1], dest)
	return err
}
...
func (i *Client) readJob(args []string) (jobId uint64, jobData []byte, err error) {
	...
	var jobDataLen int64
	jobDataLen, err = strconv.ParseInt(args[1], 10, 64)
	if err != nil {
		return
	}

	dataLen := jobDataLen + 2
	jobData = make([]byte, dataLen)
	var n int
	var startN int64
	for startN < dataLen {
		n, err = i.ReadWriter.Read(jobData[startN:])
		if err != nil {
			return
		}
		startN += int64(n)
	...
	if startN != dataLen {
		err = exception(fmt.Sprintf("read only %d bytes of %d. ERR: %v", n, dataLen, err))
		return
	}

	jobData = jobData[:dataLen-2]
	return
}

func (i *Client) Watch(tubeName string) (err error) {
	_, err = i.wordsCmd(fmt.Sprintf(msgWatch, tubeName), WATCHING)
	return
}

func (i *Client) Bury(jobId uint64, priority uint32) (err error) {
	_, err = i.wordsCmd(fmt.Sprintf(msgBury, jobId, priority), BURIED)
	return
}
...
func (i *Client) KickJob(jobID uint64) (err error) {
	_, err = i.wordsCmd(fmt.Sprintf(msgKickJob, jobID), KICKED)
	return
}
...
func (i Client) Use(tubeName string) error {
	_, err := i.wordsCmd(fmt.Sprintf(msgUse, tubeName), USING)
	return err
}
